...
impl RemoteTracer {
    fn new(hub_addr: SocketAddr, watcher: Address, callback_address: Address) -> Self {
        let route = Route::new()
            .append(format!("1#{}", hub_addr))
            .append("tracing_service")
            .into();
    ...
    pub async fn create<A: Into<Address>, S: Into<String>>(
        ...
        if let Ok(hub_addr) = hub_addr.into().parse::<SocketAddr>() {
            ...
            let tracer = Self::new(hub_addr, watcher.into(), child_ctx.address());
